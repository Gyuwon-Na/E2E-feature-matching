...
def main():
    ...
    for i in range(min(5, len(img_list))):
        ...
        plt.imshow(res['img_vis'])
        plt.title(f"Angle: {res['angle']:.1f}° | Error: {res['error']:.2f} px")
        
        # 점 찍기
        offset = np.array([res['w'], 0])  # 오른쪽 이미지로 좌표 이동
        
        # 1. 왼쪽 이미지(Source) 위의 시작점 (초록색 점)
        plt.scatter(res['src_pts'][:,0], res['src_pts'][:,1], 
                   c='lime', s=10, label='Start', zorder=3)
        
        # 2. 오른쪽 이미지(Target) 위의 정답 (파란 X)
        gt_pts = res['dst_gt'] + offset
        plt.scatter(gt_pts[:,0], gt_pts[:,1], 
                   c='blue', marker='x', s=50, label='GT', zorder=3)
        
        # 3. 오른쪽 이미지(Target) 위의 예측 (빨간 점 + 선)
        pred_pts = res['dst_pred'] + offset
        plt.scatter(pred_pts[:,0], pred_pts[:,1], 
                   c='red', s=20, label='Pred', zorder=3)
        
        # 선 긋기 (Start -> Pred)
        for j in range(len(res['src_pts'])):
            p1 = res['src_pts'][j]
            p2 = pred_pts[j]
            plt.plot([p1[0], p2[0]], [p1[1], p2[1]], 
                    c='red', alpha=0.3, linewidth=0.5, zorder=1)
