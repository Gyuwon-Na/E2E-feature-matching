...
ANGLE_THRESHOLD = 60.0   # [v2] 테스트 각도 범위 확장
...
def invert_affine_norm(matrix_2x3):
    """2x3 Normalized Affine의 역행렬 구하기 (Torch Tensor)"""
    # 3x3으로 확장 후 역행렬 계산
...
def evaluate_with_mpc(img_path, model_components):
    ...
    # 2. Random Warp (문제 출제 - Large Angle Test)
    ...
    W_gt_pixel = np.linalg.inv(M_warp_aug)[:2, :] # Source -> Target (Inverse of Warp)

    # 3. Phase 1~3 Inference (Coarse Estimation)
    preprocessor = MathGeometricPreprocessor()
    # Level 3까지 뽑아서 Phase 4에 충분한 Coarse 정보 제공
    ...
    # -------------------------------------------------------------------------
    # 4. Phase 4 v2: Hierarchical MPC Refinement
    # -------------------------------------------------------------------------
    print(f"  [MPC] Hierarchical Refining (Angle: {angle:.1f})...")
    
    # (1) Handover Preparation
    # Phase 3는 Forward Matrix를 예측하지만, 
    # MPC(grid_sample)는 Backward Mapping(Inverse)을 사용해야 함.
    # 따라서, P3 결과를 뒤집어서 초기값(W_init)으로 설정
    W_p3_inv = invert_affine_norm(W_p3_norm)
    
    # (2) Run Optimization
    refiner = HierarchicalMPCRefiner(device=DEVICE)
    
    # Phase 2 Features (p2_a, p2_b) 리스트를 그대로 전달
    W_mpc_inv, loss_curve = refiner.optimize(p2_a, p2_b, W_init=W_p3_inv)
    
    # (3) Final Result Extraction
    # Refiner 결과는 Inverse Matrix이므로, 평가를 위해 다시 Forward로 뒤집음
    W_mpc_norm = invert_affine_norm(W_mpc_inv).cpu().numpy()[0]
    W_p3_norm = W_p3_norm.cpu().numpy()[0]
...
def main():
    ...
    try:
        # weights_only=False 추가 (PyTorch 버전 호환성)
    ...
    test_count = 5 # 테스트 이미지 수
    actual_count = min(len(img_list), test_count)
    
    print(f"Testing on {actual_count} images with Hierarchical MPC...\n")
