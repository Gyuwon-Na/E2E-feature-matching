...
class IterativeRefinementLoop(nn.Module):
    ...
    def forward(self, pyramid_features_a, pyramid_features_b, phase3_results, device):
        ...
        if phase3_results is not None and len(phase3_results) > 0:
            ...
            # (1) Phase3가 제공하는 누적 글로벌 변환(θ_B2A)이 있으면 그걸 최우선 사용
            init_W = None
            for key in ('W_B2A', 'W_global'):
                if isinstance(coarse_res, dict) and coarse_res.get(key, None) is not None:
                    init_W = coarse_res[key]
                    break

            # (2) fallback: rotor_map 평균으로 θ 생성
            if init_W is None:
